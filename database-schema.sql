-- Ananda College Karate Club - Complete Database Schema
...
-- ============================================================================
-- The old approach computed admission IDs client-side ("ACK-2026-001"), which
-- could hand two coaches the same ID if they added students at the same
-- instant (and the UNIQUE constraint then rejected one insert). Now the DB
-- assigns them atomically via a per-year counter — no races possible.
...
-- Uncomment the following to add sample training sessions:

-- INSERT INTO sessions (name, days, time) VALUES
--   ('Morning Training', ARRAY[1,3,5], '06:00'),
--   ('Evening Training', ARRAY[1,3,5], '16:30'),
--   ('Saturday Practice', ARRAY[6], '08:00');
...
-- ============================================================================
-- MIGRATION (existing databases — de-gamification)
-- Drops the synthetic point values that are no longer used by the app.
-- Placements (Gold/Silver/Bronze/etc.) stay — they are real result data.
-- ============================================================================

-- ALTER TABLE achievements DROP COLUMN IF EXISTS points;
-- ALTER TABLE club_settings DROP COLUMN IF EXISTS weight_attendance;
-- DELETE FROM club_settings WHERE id = 1 AND weight_attendance IS NULL; -- no-op guard
